...
class NeuralNet:
    def __init__(self, NN_name, dim_layers):
        ...
        with tf.variable_scope(self.NN_name, reuse=tf.AUTO_REUSE):
            ...
            self.loss(dA=self.dA, YLabel=self.YLabel,
                      Y=self.Y, gradY=self.gradY)
            self.optimizer()
    ...
    def build_hyperparams(self):
        ...
        self.rate = tf.cast(tf.train.exponential_decay(
            self.init_rate, global_step=tf.clip_by_value(
                self.step - self.n_relaxstep, 0, self.n_decaystep),
        ...
        self.bn_rate = bn_init_rate * (tf.cast(tf.train.exponential_decay(
            bn_init_rate, global_step=tf.clip_by_value(
                self.step, 0, self.n_decaystep),
            decay_steps=self.n_decaystep, decay_rate=self.decay_rate, staircase=False), dtype=tf_floattype)
    ...
    def build_dataset(self):
        ...
        '''
        self.ph_X = tf.placeholder(dtype=tf_floattype, shape=[
                                   None, None, d], name="X")
        self.ph_dA = tf.placeholder(dtype=tf_floattype, shape=[
                                    None, None, d], name="dA")
        self.ph_YLabel = tf.placeholder(dtype=tf_floattype, shape=[
                                        None, None, 1], name="YLabel")
        self.ph_Y_prestep = tf.placeholder(dtype=tf_floattype, shape=[
                                           None, None, 1], name="Y_prestep")
        self.ph_gradY_prestep = tf.placeholder(
            dtype=tf_floattype, shape=[None, None, d, 1], name="gradY_prestep")
        ...
        test_iter = tf.data.Iterator.from_structure(
    ...
    def __batch_norm_layer(self, batch_norm_name, Z, input_mv=False, tol=1e-8):
        ...
        with tf.variable_scope(batch_norm_name, reuse=tf.AUTO_REUSE):
            ...
            batch_mean, batch_var = tf.nn.moments(Z, axes=list(
                range(len(Z.get_shape())-2)), keep_dims=True)
            if input_mv:
                input_mv_init_op = [
                    tf.assign(mv_mean, batch_mean), tf.assign(mv_var, batch_var)]
                Z = tf.nn.batch_normalization(
                    Z, mv_mean, mv_var, beta, gamma, tol)
            ...
            with tf.variable_scope("layer" + str(layer), reuse=tf.AUTO_REUSE):
                if layer == 1:
                    ...
                    WX = tf.get_variable("WX", dtype=tf_floattype,
                                         shape=[num_channels, d,
                                                self.dim_layers[1]],
                                         initializer=tf.random_uniform_initializer(-lim, lim), trainable=True)
                    lim = stddev / \
                        math.sqrt(self.dim_layers[0] + self.dim_layers[1])
                    WZ = tf.get_variable("WZ", dtype=tf_floattype,
                                         shape=[
                                             num_channels, self.dim_layers[0], self.dim_layers[1]],
                                         initializer=tf.random_uniform_initializer(-lim, lim), trainable=True)

                if layer >= 2:
                    lim = stddev / \
                        math.sqrt(
                            self.dim_layers[layer - 1] + self.dim_layers[layer])
                    WZ = tf.get_variable("WZ", dtype=tf_floattype,
                                         shape=[
                                             num_channels, self.dim_layers[layer - 1], self.dim_layers[layer]],
                                         initializer=tf.random_uniform_initializer(-lim, lim), trainable=True)

                self.__batch_norm_layer_variables(
                    "batchZ", self.dim_layers[layer])
    ...
    def build_graph(self, graph_type, X, Y_prestep, gradY_prestep, sharpness):
        ...
        with tf.variable_scope("layer" + str(layer), reuse=tf.AUTO_REUSE):
            ...
            input_mv_init_op, X_normal = self.__batch_norm_layer(
                "batchX", X, input_mv=True)

            Z = tf.concat([Y_prestep, L], axis=-1)
            input_mv_init_op_Z, Z_normal = self.__batch_norm_layer(
                "batchZ", Z, input_mv=True)
        ...
        for layer in range(1, self.num_layers - 1):
            with tf.variable_scope("layer" + str(layer), reuse=tf.AUTO_REUSE):
                ...
                if layer == 1:
                    WX = tf.get_variable("WX", dtype=tf_floattype)
                    WZ = tf.get_variable("WZ", dtype=tf_floattype)
                    Z = self.__tensor_contract(
                        X_normal, WX) + self.__tensor_contract(Z_normal, WZ)
        ...
        gradY = tf.expand_dims(tf.gradients(Y, X)[0], axis=-1) \
            + self.__tensor_chainrule(tf.gradients(Y,
                                      Y_prestep)[0], gradY_prestep)

        if graph_type == "train":
            return Y, gradY
        elif graph_type == "test":
            ensemble_Y = tf.reduce_mean(Y, axis=1, keepdims=True)
            ensemble_gradY = 1 / num_channels * \
                tf.reduce_mean(gradY, axis=1, keepdims=True)
    ...
    def loss(self, dA, YLabel, Y, gradY):
        ...
        Res = YLabel - Y - math.exp(-r * dt) * \
            tf.reduce_sum(dA * gradY, axis=-2)
    ...
    def check_graph(self):
        ...
        '''
        print("\n Trainable variables in the main neural network ",
              self.NN_name, ": ")
        ...
        for step in range(n_totalstep):
            if step < n_unitstep or step % n_unitstep == 0 or step == n_totalstep - 1:
                glstep, lr, br, b, gradY, regloss, _ = \
                    sess.run([self.step, self.rate, self.bn_rate,
                             self.ensemble_b, self.gradY, self.Loss, self.Optimizer])
    ...
    def train(self, sess, n, AmerOp, n_totalstep, n_unitstep, n_relaxstep, n_decaystep,
        ...
        sess.run([tf.assign(self.delta, updaten - self.n % updaten),
                  tf.assign(self.init_rate, init_rate), tf.assign(
        ...
        dAn = customer_reshape(
            AmerOp.X.values[simulation_index, self.n + 1]) - np.exp(mu * dt) * Xn
        YLabeln = customer_reshape(
            AmerOp.YLabel.values[simulation_index, self.n])
        Y_prestepn = customer_reshape(
            AmerOp.Y.values[simulation_index, self.n])
        gradY_prestepn = customer_reshape(
            AmerOp.gradY.values[simulation_index, self.n])
        ...
        # Main Optimization
        t0 = time.time()
        self.fit_Optimizer(sess, Xn, dAn, YLabeln, Y_prestepn,
                           gradY_prestepn, n_totalstep, n_unitstep)
    ...
    def predict_Y_gradY_timestep(self, sess, AmerOp, m):
        ...
        AmerOp.Y.values[:, [m]], AmerOp.gradY.values[:, [m]] = sess.run(
            [self.ensemble_Y, self.ensemble_gradY])
